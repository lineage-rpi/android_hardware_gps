...
To set serial port, add a property "persist.vendor.gps.device" and set it equal to your GPS device file.
ie. persist.vendor.gps.device=ttyO1

Default baud rate is 9600, to adjust add a property "persist.vendor.gps.ttybaud" and set it equal to the needed rate. (4800-115200)
ie. persist.vendor.gps.ttybaud=9600
...
to modify by property "persist.vendor.gps.max_rate" if your GPS device is able to process UBX protocol. Acceptable property value
...
"persist.vendor.gps.time_sync" which is greater than zero is taken as threshold in seconds. If difference between system time
